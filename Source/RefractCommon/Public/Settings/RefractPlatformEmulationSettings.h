...
#include "RefractPlatformEmulationSettings.generated.h"
 ...
 */
UCLASS(MinimalAPI, Config = EditorPerProjectUserSettings, DisplayName = "Platform Emulation Settings (Editor)")
class URefractPlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UE_API URefractPlatformEmulationSettings();

	//~UDeveloperSettings interface
	virtual FName GetCategoryName() const override;
	//~End of UDeveloperSettings interface

	UE_API FName GetPretendBaseDeviceProfile() const;
	UE_API FName GetPretendPlatformName() const;

protected:
	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (Categories = "Input, Platform.Trait"))
	FGameplayTagContainer AdditionalPlatformTraitsToEnable;

	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (Categories = "Input, Platform.Trait"))
	FGameplayTagContainer AdditionalPlatformTraitsToSuppress;

	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (GetOptions = GetKnownPlatformIds))
	...
	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (GetOptions = GetKnownDeviceProfiles, EditCondition = bApplyDeviceProfilesInPIE))
	...
	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (ConsoleVariable = "Refract.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (ConsoleVariable = "Refract.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(Category = "Platform Emulation", EditAnywhere, Config, meta = (InlineEditConditionToggle, ConsoleVariable = "Refract.Settings.ApplyDeviceProfilesInPIE"))
